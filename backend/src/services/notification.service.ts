...
export async function bulkCreateNotifications(
  ...
  payload: Omit<NotificationRow, 'userId'>,
) {
...
export async function notifyNovelAndAuthorFollowers(
  ...
  const novelFollowers = await prisma.follow.findMany({
    where: { novelId },
    select: { userId: true },
  });
  ...
  const authorFollowers = await prisma.userFollow.findMany({
    where: { authorId: novel.authorId },

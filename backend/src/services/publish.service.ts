...
const LANG_DETECTION_RATIO = getEnvNumber(
  cfg.LANG_DETECTION_RATIO,
  process.env.LANG_DETECTION_RATIO,
  0.6,
);
...
export async function runPrePublishChecks(novelId: number): Promise<PublishCheckResult> {
    ...
    typeof novel.wordCount === 'number' && !Number.isNaN(novel.wordCount)
      ...
      : Array.isArray(novel.chapters)
        ? novel.chapters.reduce(
            (s, c) => s + (c.wordCount ?? countWordsFromHtml(c.content ?? '')),
            0,
          )
        : 0;
  ...
  if (LANG_DETECTION_ENABLED) {
    details.langDetection = {
      note: 'Per-chapter language checks are performed during chapter creation.',
    };
  }

  // 4) require at least 1 genre and 1 tag
  const genreCount = Array.isArray(novel.genres)
    ? novel.genres.length
    : await prisma.novelGenre.count({ where: { novelId } });
  const tagCount = Array.isArray(novel.tags)
    ? novel.tags.length
    : await prisma.novelTag.count({ where: { novelId } });

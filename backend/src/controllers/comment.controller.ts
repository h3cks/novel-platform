...
export const createCommentForNovel = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (err.code === 'PARENT_NOT_FOUND')
      return fail(res, 404, 'PARENT_NOT_FOUND', 'Parent comment not found');
    if (err.code === 'MISSING_TARGET')
      return fail(
        res,
        400,
        'MISSING_TARGET',
        'Either novelId or chapterId or parentId is required',
      );
    throw err;
  }
});
...
export const createCommentForChapter = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (err.code === 'PARENT_NOT_FOUND')
      return fail(res, 404, 'PARENT_NOT_FOUND', 'Parent comment not found');
    if (err.code === 'NOVEL_NOT_FOUND' || err.code === 'CH_NOT_FOUND')
      return fail(res, 404, 'TARGET_NOT_FOUND', err.message);
    throw err;
  }
});

...
const ALLOWED_STATUS = new Set<ReportStatusLiteral>([
  'OPEN',
  'IN_PROGRESS',
  'RESOLVED',
  'DISMISSED',
...
export const createReport = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!targetType || !targetId || !reason) {
    return fail(
      ...
      400,
      'MISSING_FIELDS',
      'Missing fields (targetType, targetId, reason) required',
    );
  }
  ...
  if (!Number.isInteger(tid) || tid <= 0)
    return fail(res, 400, 'INVALID_TARGET_ID', 'Invalid targetId');
  ...
  try {
    ...
    if (err.code === 'ALREADY_REPORTED') {
      return fail(
        ...
        409,
        'ALREADY_REPORTED',
        'Report already exists and is not resolved/dismissed',
      );
    }
    throw err;
  }
...
export const listReports = asyncHandler(async (req: Request, res: Response) => {
  ...
  const targetTypeRaw =
    typeof req.query.targetType === 'string' ? req.query.targetType.toLowerCase() : undefined;
  const targetType =
    targetTypeRaw && ['novel', 'chapter', 'comment', 'user'].includes(targetTypeRaw)
      ? (targetTypeRaw as any)
      : undefined;
...
export const processReport = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (status !== undefined && statusEnum === undefined)

...
export const createChapter = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!Number.isInteger(novelId) || novelId <= 0)
    return fail(res, 400, 'INVALID_NOVEL_ID', 'Invalid novelId');
  ...
  if (wc < MIN_WORDS_PER_CHAPTER) {
    return fail(
      ...
      400,
      'CHAPTER_TOO_SHORT',
      `Chapter must have at least ${MIN_WORDS_PER_CHAPTER} words after sanitization`,
    );
  }

  try {
    const chapter = await chapterService.createChapter(novelId, user.id, {
      title: String(title).trim(),
      content: cleanContent,
    });
    return ok(res, { chapter }, undefined, 201);
  } catch (err: any) {
    if (err?.code === 'NOVEL_NOT_FOUND')
      return fail(res, 404, 'NOVEL_NOT_FOUND', 'Novel not found');
...
export const getChapter = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (chapter.novelId !== novelId)
    return fail(res, 400, 'CHAPTER_MISMATCH', 'Chapter does not belong to this novel');
...
export const updateChapter = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (typeof title !== 'undefined') {
    if (!validateTitle(title))
      return fail(res, 400, 'INVALID_TITLE', 'Invalid title (1-250 chars)');
  }
  if (typeof content !== 'undefined') {
    ...
    if (wc < MIN_WORDS_PER_CHAPTER) {
      return fail(
        ...
        400,
        'CHAPTER_TOO_SHORT',
        `Chapter must have at least ${MIN_WORDS_PER_CHAPTER} words`,
      );
    }
  }
